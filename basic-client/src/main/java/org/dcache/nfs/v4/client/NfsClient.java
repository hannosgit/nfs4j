...
public class NfsClient implements AutoCloseable {
    ...
    public NfsClient(int port, @Nonnull NfsVersion nfsVersion, @Nonnull String server, @Nonnull String export) throws IOException, OncRpcException, InterruptedException {
        ...
        this.nfsClient = new NfsClient(serverAddress);
    ...
    private static class LeaseUpdater implements Runnable {
        ...
        private final NfsClient _nfsClient;

        LeaseUpdater(NfsClient nfsClient) {
    ...
    private void read(String path, boolean pnfs) throws OncRpcException, IOException {
        ...
        if (pnfs && _isMDS) {
            ...
            NfsClient dsClient = _servers.getUnchecked(deviceAddr);
    ...
    private final LoadingCache<InetSocketAddress, NfsClient> _servers =
            CacheBuilder.newBuilder().build(new Connector());

    private static class Connector extends CacheLoader<InetSocketAddress, NfsClient> {

        @Override
        public NfsClient load(InetSocketAddress f) {
            try {
                NfsClient client = new NfsClient(f);
                client.dsMount();
                return client;
            } catch (Exception e) {
                return null;
